...
HRESULT CMeshLoader::LoadGeometryFromOBJ(const wstring& strFileName)
{
	...
	for (; ; )
	{
		...
		{
			// Material library
			wstring wTemp = strFileName;
			int number = wTemp.rfind('/');
			wTemp = wTemp.substr(0, number + 1);

			InFile >> strMaterialFilename;

			strMaterialFilename = wTemp + strMaterialFilename;
		}
...
HRESULT CMeshLoader::LoadMaterialsFromMTL(const wstring& strFileName)
{
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"map_Kd"))
		{
			...
			wstring wTemp = strFileName.substr(0, number + 1);

			pMaterial->strTexture = wTemp.c_str() + tempstr;
			

			number = pMaterial->strTexture.rfind('.');
			pMaterial->strTexture = pMaterial->strTexture.substr(0, number);
			pMaterial->strTexture = pMaterial->strTexture + L".png";

			pMaterial->pTexture = IMAGE->GetTexture(pMaterial->strTexture, pMaterial->strTexture)->texturePtr;
		}
